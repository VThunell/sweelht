...
sweelsilv.code <- nimbleCode({
  ...
  sd_a_su ~ dgamma(shape = 2, rate = 8)  # sd for su-level
  sd_b_su ~ dgamma(shape = 2, rate = 8)  # sd for su-level
...
sweelsilv.model <- nimbleModel(sweelsilv.code,
                               constants = const,
                               inits=inits(),
                               data = df.sweel3 %>% select(silver_vis,silver_dur,length_sc,age_sc), 
                               buildDerivs = TRUE,
                               calculate = FALSE)
