...
sweelsilv.code <- nimbleCode({
  ...
  for(i in 1:nobs){
    ...
    age_sc[i] ~ dnorm(0, 1)
    weight_sc[i] ~ dnorm(0, 1)
    #length_sc[i] ~ dnorm(0, 1)
    
    # logistic regression
    z[i] <- alpha[su[i]] + b0[su[i]]*length_sc[i] + b1*age_sc[i] + b2*weight_sc[i] + b3*lat_sc[i] + b4[sb[i]]*marine[i]
    p[i] <- 1 / (1 + exp(-z[i]))
    silver[i] ~ dbern(p[i])
...
sweelsilv.model <- nimbleModel(sweelsilv.code,
                               constants = const,
                               inits=inits(),
                               data = df.sweel3 %>% select(silver_vis,silver_dur,length_sc,age_sc,lat_sc,marine,weight_sc), 
                               buildDerivs = TRUE,
                               calculate = FALSE)
...
monits = c(mvars,"p","z","alpha","logb0","logProb_silver_vis","logProb_silver_dur")
...
saveRDS(sweelsilv.samples, file = paste0(home,"/data/samples/sweelsilv.samples_e",Sys.Date(),".RData"))
