...
sweelsilv.code <- nimbleCode({
  ...
  for(j in 1:nsb){
    alpha[j] ~ dnorm(mu_a_gl, sd = sd_a_gl/2)
    #alpha[j] ~ dnorm(mu_a_gl, sd = sd_a_gl) # for non-nested alpha
    log_b0[j] ~ dnorm(mu_b_gl, sd = sd_b_gl/2)
  ...
  for(i in 1:nobs){
    ...
    z[i] <- alpha[sb[i]] + b0[sb[i]]*length_sc[i] + b1*age_sc[i]
    p[i] <- 1 / (1 + exp(-z[i]))
    silver[i] ~ dbern(p[i])
...
inits <- function() {list(
    alpha = rnorm(const$nsb, 0, 1),
    log_b0  = rnorm(const$nsb, 0, .1),
    log_b0 = rep(log(const$mb0), const$nsb),
