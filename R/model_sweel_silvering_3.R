...
const <- list(nobs = nobs,
              msa = (746 - mean(df.sweel3$length1, na.rm = TRUE)) / sd(df.sweel3$length1, na.rm = TRUE),
              sda = 200/sd(df.sweel3$length1, na.rm = TRUE),
              mb0 = 1,
              sdb0 = 2,
              nsu = nsu,
              su = df.sweel3$su
...
t <- Sys.time()
sweelsilv.samples <- runMCMC(sweelsilv.mcmcc, niter = 3000, nburnin = 1500, nchains = 2, WAIC = TRUE)
Sys.time() - t
